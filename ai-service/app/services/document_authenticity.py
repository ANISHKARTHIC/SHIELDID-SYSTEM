...
def assess_authenticity(
    ...
    # 1. OCR Confidence Contribution (40% weight)
    ocr_score = min(100.0, max(0.0, ocr_confidence))
    if ocr_score < 80:
        issues.append(f"Low text confidence ({ocr_score:.1f}%)")

    # 2. Dynamic Microtext / Edge Sharpness Contribution (35% weight)
    ...
    # 3. Print Quality Contribution (25% weight)
    ...
    # Weighted average of the three signal scores, minus a flat blur
    # penalty. Previously this weighted average (base_score) was followed
    # by a second, independent penalty pass that re-penalized the same
    # three metrics (ocr_score/edge_score/print_score) a second time on
    # different thresholds — double-counting the same weak signal — and
    # then the result was perturbed by a deterministic MD5-hash-derived
    # "natural_variance" and remapped into hardcoded percentage bands
    # purely to look less static ("more human-like"). None of that made
    # the score more accurate; it made it less deterministic and harder to
    # reason about. This is a single, honest weighted score instead.
    final_score = max(0.0, min(100.0,
        (ocr_score * 0.40) + (edge_score * 0.35) + (print_score * 0.25) - blur_penalty
    ))
